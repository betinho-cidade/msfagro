            ...
            <div class="card-body">
                ...
                <ul class="nav nav-tabs" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" data-toggle="tab" href="#lancamento_R" role="tab">
                            ...
                            <span class="d-none d-sm-block">Receita ( <code class="highlighter-rouge">{{$movimentacaos_MF->where('tipo', 'R')->count()}}</code> )
                                @can('delete_list_lancamento')
                                    @if($movimentacaos_MF->where('tipo', 'R')->count() > 0)
                    ...
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#lancamento_D" role="tab">
                            <span class="d-block d-sm-none"><i class="ri-checkbox-circle-line"></i></span>
                            <span class="d-none d-sm-block">Despesa ( <code class="highlighter-rouge">{{$movimentacaos_MF->where('tipo', 'D')->count()}}</code> )
                                @can('delete_list_lancamento')
                                    @if($movimentacaos_MF->where('tipo', 'D')->count() > 0)
                ...
                <div class="tab-pane active" id="lancamento_R" role="tabpanel">
                    <table id="dt_lancamentos_R" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        <tbody>
                        @forelse($movimentacaos_MF->where('tipo', 'R') as $movimentacao)
                        <tr>
                            ...
                            <td data-toggle="tooltip" title="{{ $movimentacao->item_fiscal }}">{{$movimentacao->item_fiscal_reduzido}}</td>
                            <td data-toggle="tooltip" title="{{ $movimentacao->categoria->nome ?? '...' }}">{{ $movimentacao->categoria->nome_reduzido ?? '...' }}</td>
                            <td>{{$movimentacao->valor}}</td>
                            <td>{{$movimentacao->data_programada_formatada}}</td>
                            <td style="text-align:center;">

                            @can('edit_lancamento')
                                <a href="{{route('lancamento.show_MF', compact('movimentacao'))}}"><i class="fa fa-edit" style="color: goldenrod" title="Editar a Movimentação Fiscal"></i></a>
                            @endcan
                ...
                <div class="tab-pane" id="lancamento_D" role="tabpanel">
                    <table id="dt_lancamentos_D" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        <tbody>
                        @forelse($movimentacaos_MF->where('tipo', 'D') as $movimentacao)
                        <tr>
                            <td>{{$movimentacao->data_programada_ordenacao}}</td>
                            <td>{{$movimentacao->id}}</td>
                            <td data-toggle="tooltip" title="{{ $movimentacao->empresa->nome_empresa ?? '...' }}">{{ $movimentacao->empresa->nome_empresa_reduzido ?? '...' }}</td>
...
@section('script-js')
   ...
   <!-- Datatable init js -->
    ...
    <script>
       ...
       {
           var id = id;
           var url = '{{ route("lancamento.destroy_MF", ":id") }}';
    ...
    <script>
        ...
        {
            var tipo = tipo;
            var url = '{{ route("lancamento.destroy_list_MF") }}';
