...
<html lang="en"><head>
  ...
  <link rel="mask-icon" type="" href="https://cpwebassets.codepen.io/assets/favicon/logo-pin-8f3771b1072e3c38bd662872f6b673a722f4b3ca2421637d5596661b4e2132cc.svg" color="#111">
    ...
    <title>Distribuição de Lucros no Período</title>
  ...
  <body translate="no">
    ...
    <div class="tab-pane active" id="lancamento" role="tabpanel">
                    <table id="dt_lancamentos" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        <tr style="background-color:#CECECE">
                            <th style="align:center">Data Lançamento</th>
                            <th style="align:center">Produtor</th>
                            <th style="align:center">Forma Pagamento</th>
                            <th style="align:center">Valor</th>
                            <th style="align:center">Comprovante</th>
                            <th style="align:center">Obs</th>
                        </tr>
                        </thead>

                        <tbody>
                        @foreach($lucros as $lucro)
                        <tr style="background-color: #73FD76">
                            <td style="text-align:center;font-size:12px;">{{$lucro->data_lancamento_formatada}}</td>
                            <td style="text-align:center;font-size:12px;">{{ $lucro->produtor->nome_produtor ?? '...' }}</td>
                            <td style="text-align:center;font-size:12px;">{{ $lucro->forma_pagamento->forma ?? '...'}}</td>
                            <td style="text-align:center;font-size:12px;">R$ {{number_format($lucro->valor, 2, ',', '.')}}</td>
                            <td style="text-align:center;font-size:12px;">
                              @if($lucro->link_comprovante)
                                <a href="{{$lucro->link_comprovante_guest}}" title="Download Comprovante Pagamento"><img src="https://login.mfsagro.com.br/images/comprovante-pagamento.png" style="max-width:17px;" alt="Download Comprovante Pagamwento"></a>
                              @endif
                            </td>
                            <td style="text-align:left;font-size:12px;">{{ $lucro->observacao ?? '...'}}</td>
                        </tr>
                        @endforeach
                        </tbody>
                    </table>
    ...
    <div class="tab-pane active" id="lancamento" role="tabpanel">
                    <table id="dt_lancamentos" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        <tbody>
                        <tr>
                            <td style="text-align:center;color: '#548235'">R$ {{number_format($resultado_final['total'], 2, ',', '.')}}</td>
                        </tr>
                        </tbody>
                    </table>
                    <!-- Nav tabs - LISTA lancamento - ATIVA - FIM -->
